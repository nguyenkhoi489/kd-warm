...
final class SiteInstallTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        tmp = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("kdwarm-install-\(UUID().uuidString)")
    ...
    func testCreateDatabaseFailsOnExistingDatabase() async throws {
        try XCTSkipUnless(ProcessInfo.processInfo.environment["KDWARM_DB_IT"] == "1",
                          "Set KDWARM_DB_IT=1 with the MySQL engine running on :3306.")
        let provisioner = DatabaseProvisioner(ensureEngine: {})
        let name = "kdwarm_it_\(UUID().uuidString.prefix(8))"

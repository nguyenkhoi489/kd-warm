...
/// `KDWARM_DB_IT=1` AND an installed engine running on :3306, so a clean CI box skips. Proves the
...
final class MySQLDriverCRUDTests: XCTestCase {
    ...
    func testInsertReadUpdateDeleteRoundTrip() async throws {
        ...
        try await driver.insert(database: schema, table: "t",
            values: [ColumnValue(column: "id", value: .int(1)),
        ...
        try await driver.update(database: schema, table: "t",
        ...
        try await driver.delete(database: schema, table: "t",
            key: [ColumnValue(column: "id", value: .int(1))])
        page = try await driver.paginatedRows(database: schema, table: "t", limit: 10, offset: 0)
    ...
    func testWriteAffectingMultipleRowsRollsBack() async throws {
        ...
        do {
            try await driver.update(database: schema, table: "t3",

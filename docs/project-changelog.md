...
### Fixed
- **Sites dashboard redesign** — Sites now uses a screenshot-aligned native SwiftUI header, site-count pill, server status pill, always-visible search, rounded table surface, icon tiles, explicit per-row status, stable trailing controls, and a primary New Site split action while preserving existing site callbacks.
- **Sites dashboard sidebar shell** — sidebar now renders the KTStack identity row with Pro capsule, live Sites count badge, and a footer server card showing Server Running/Stopped plus the bundle version. Site rows split runtime, status, and overflow controls into focused subviews, and the tunnel share control adds accessibility labels on icon-only buttons.
- **Remove Site destructive cleanup** — removing a site now opens a parent-level destructive alert, deletes the registered folder from disk, removes the registry entry, and drops the tracked MySQL database for newly-created managed sites.

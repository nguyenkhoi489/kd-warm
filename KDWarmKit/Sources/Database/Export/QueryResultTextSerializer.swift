...
public enum QueryResultTextSerializer {
    public static func csv(_ result: QueryResult, rows indices: [Int]? = nil,
                           includeHeaders: Bool = true) -> String {
        serialize(result, rows: indices, includeHeaders: includeHeaders,
                  delimiter: ",", lineEnding: "\r\n")
    }

    public static func tsv(_ result: QueryResult, rows indices: [Int]? = nil,
                           includeHeaders: Bool = false) -> String {
        serialize(result, rows: indices, includeHeaders: includeHeaders,
                  delimiter: "\t", lineEnding: "\n")
    }

    private static func serialize(_ result: QueryResult, rows indices: [Int]?,
                                  includeHeaders: Bool, delimiter: String,
        ...
        for index in rowIndices where result.rows.indices.contains(index) {
            lines.append(result.rows[index]

...
public struct Port53ConflictDetector {
    ...
    static func message(for process: String) -> String {
        ...
        case let p where p.contains("herd") || p.contains("valet"):
            return "Another local-dev DNS tool (\(process)) is using port 53. Stop Herd/Valet, then enable KDWarm DNS."
        default:
            return "Port 53 is already in use by “\(process)”. Stop it, then enable KDWarm DNS."
        }
    }

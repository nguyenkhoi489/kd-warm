...
public struct AppSupportPaths: Sendable {
    ...
    public init(fileManager: FileManager = .default) {
        ...
        root = base.appendingPathComponent("KTStack", isDirectory: true)
    }
    ...
    public var phpConfigDir: URL {
        config.appendingPathComponent("php", isDirectory: true)
    }

    public var sitesConfigDir: URL {
        config.appendingPathComponent("sites", isDirectory: true)
    }

    public var caDir: URL {
        config.appendingPathComponent("ca", isDirectory: true)
    }

    public var certsDir: URL {
        config.appendingPathComponent("certs", isDirectory: true)
    }
    ...
    public var sitesRegistryFile: URL {
        sitesConfigDir.appendingPathComponent("sites.json")
    }
    ...
    public func tunnelLabel(_ siteID: String) -> String {
        "com.ktstack.tunnel.\(siteID)"
    }
    ...
    public var backupManifest: URL {
        backups.appendingPathComponent("manifest.json")
    }

    public var queryHistoryFile: URL {
    ...
    public var restoreStagingRoot: URL {
        dir("restore-staging")
    }
    ...
    public var composerPhar: URL {
        toolsDir("composer").appendingPathComponent("composer.phar")
    }

    public var wpCliPhar: URL {
        toolsDir("wp-cli").appendingPathComponent("wp-cli.phar")
    }

    public var dumpsPrependFile: URL {
    ...
    public var caRootCert: URL {
        caDir.appendingPathComponent("rootCA.pem")
    }

    public var caRootKey: URL {
        caDir.appendingPathComponent("rootCA-key.pem")
    }

    public var catchAllCert: URL {
        caDir.appendingPathComponent("catchall-cert.pem")
    }

    public var catchAllKey: URL {
        caDir.appendingPathComponent("catchall-key.pem")
    }

    public func siteCertDir(_ name: String) -> URL {
        certsDir.appendingPathComponent(name, isDirectory: true)
    }

    public func siteCert(_ name: String) -> URL {
        siteCertDir(name).appendingPathComponent("cert.pem")
    }

    public func siteKey(_ name: String) -> URL {
    ...
    public var nginxPid: URL {
        run.appendingPathComponent("nginx.pid")
    }

    public var nginxErrorLog: URL {
        logs.appendingPathComponent("nginx-error.log")
    }

    public var nginxAccessLog: URL {
        logs.appendingPathComponent("nginx-access.log")
    }

    public var logsSites: URL {
        logs.appendingPathComponent("sites", isDirectory: true)
    }

    public func siteAccessLog(_ domain: String) -> URL {
        logsSites.appendingPathComponent("\(domain).access.log")
    }

    public func siteErrorLog(_ domain: String) -> URL {
